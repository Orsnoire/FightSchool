...
- **Dynamic Enemy HP Scaling**: Enemy HP scales automatically based on party size and player progression. Formula: `Base HP = playerCount × (2.5 + avgJobLevel × 0.4) × difficultyMultiplier`. The difficultyMultiplier directly represents the approximate number of questions an enemy should survive at 70% player accuracy. Teachers set a difficulty multiplier (1-100) per enemy: difficulty 10 = ~10 questions to defeat, difficulty 5 = ~5 questions, etc. This ensures fights scale predictably from 2-30 players, and teachers can adjust difficulty to match player power level (higher-level parties need higher difficulty for the same fight duration).
