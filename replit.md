...
- **Solo Mode**: Students can practice fights alone or with friends. Solo sessions have level-based HP scaling using the formula: `10 HP base + (2 × playerCount) + (5 × totalLevelsAbove2)`, capped at the enemy's maxHealth from the fight template. Host controls allow enabling AI for absent players and blocking new joiners mid-session.
